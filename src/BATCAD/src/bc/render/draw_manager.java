...
public class draw_manager {
  ...
  public void free ()
  {
	...
	draw_cw renderer = new draw_cw(this);
        GLPbuffer = renderer.WaitForImage(); // optimally this will be done as late as possible before the next frame,
...
The application requests create_instace("imagefile.svg") from the rendering subsystem.
The renderer checks to see if that image is already in memory.
If it isn't, the renderer loads the image from its source file and breaks it into components.
The draw_instance is made to link back to the image.

Whenever the renderer cycles, it looks through the instances and draws each one's
linked image prototype to a buffer, stepping through each component primitive in the prototype,
and offsetting the primitive by (1) its place in the image, (2) the instance's position in
the environment, and (3) the environment perspective.
*/